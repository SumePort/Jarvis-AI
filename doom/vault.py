...
class LocalProtectedVault:
    ...
    def _write_key(self, key: bytes) -> None:
        if not isinstance(key, bytes):
            raise VaultError("Vault key must be bytes")
        ...
        if os.name == "nt":
            ...
            # pywin32 normally returns:
            # (description, protected_data)
            #
            # Some Python/pywin32 combinations can expose the
            # result differently, so explicitly locate the bytes.
            if isinstance(result, tuple):
                protected = next(
                    (item for item in result if isinstance(item, bytes)),
                    None,
                )
            else:
                protected = result
            ...
            if not isinstance(protected, bytes):
                raise VaultError(
                    "Windows DPAPI did not return protected bytes"
                )
    ...
    def _read_key(self) -> bytes:
        ...
        if os.name == "nt":
            ...
            if not isinstance(key, bytes):
                raise VaultError(
                    "Windows DPAPI did not return the vault key"
                )
