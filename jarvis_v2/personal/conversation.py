...
class JarvisConversation:
    """Multi-turn assistant loop connecting session, personal context, memory and brain."""
    def __init__(self, brain: BrainProvider, session: JarvisSession | None = None, personal: PersonalContext | None = None, memory: PersonalMemory | None = None):
        self.brain=brain
    ...
    def _context(self, text: str) -> dict[str, Any]:
        ...
        return {"personal":personal,"conversation":conversation,"session":{"id":self.session.session_id,"authenticated":self.session.authenticated,"active_project":self.session.active_project},"request":text}

    def handle(self, text: str) -> ConversationResult:
        if not self.session.authenticated:
            raise PermissionError("JARVIS session is not authenticated")
        text=text.strip()
        if not text: raise ValueError("Conversation input cannot be empty")
